...
test('apply creates labels, comments once, closes after comment, and leaves a second plan empty', () => {
  ...
  const calls: Call[] = [];
  const commentView = 'issue view 12 --repo owner/repo --json comments';
  const gh = fakeGh({
  ...
  const second = planGithub({ links: linked, tickets: done, released: { tk_one: { version: 'v3.546.0' } }, gh: fakeGh({ [list]: [{ number: 12, state: 'CLOSED', labels: [] }] }) });
...
test('does not close when comment lookup or posting fails', () => {
  ...
  const results = applyGithub(plan, fakeGh({
    'label list --repo owner/repo --limit 1000 --json name': [],
    'issue view 12 --repo owner/repo --json comments': { comments: [] },
