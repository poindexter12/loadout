...
- Grok rows are advertised ONLY when Grok CLI auth is present (`~/.grok/auth.json`, or `CODEX_GATEWAY_GROK_HOME`). Both the shim's boot catalog and its `refreshModels()` consult the same `getGrokReadiness()`, so the two never disagree. A missing Grok row therefore means "not signed in", not "shim broken": run `grok` and log in, then restart Claude Code. Typing `/model claude-grok-4.5[1m]` still routes and still refuses with the readGrokAuth message — gating is advertisement-only.
...
- **Lifecycle refuses an unidentified listener**: startup, stop, supervisor replacement, drain, and
  worker restart require positive shim listener ownership before cleanup or control HTTP. A bound port
  with no discoverable PID, missing command line, or failed inspection stays untouched. Retry `ensure`
...
- **Proxy recovery cannot confirm ownership**: three consecutive failed health probes are required
  ...
  records `owner-unknown` when process inspection is inconclusive, and tries again after backoff.
  Linux discovery retains the `/proc` fallback when `lsof` is unavailable. Windows discovery keeps
  probe output attached; shutdown kills and reaps probe children. A confirmed foreign owner is
  refused and never replaced. Account-specific state and socket paths remain isolated.
