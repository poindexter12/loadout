...
function releaseTicket(slug, idOrRef, by, opts) {
  ...
  return withTicketLock(slug, found.id, () => {
    ...
    const forcedTakeover = Boolean(opts.force || opts.forceClaimTakeover) && Boolean(forcedTakeoverEvidence) && heldOwner && heldOwner !== by && !claimReclaimable(t);
  ...
  if (purpose === "delivery") {
    if (ticket.claim?.by || ticket.dispatchNonce || state && !state.terminalAt) {
      return {
        ok: false,
        reason: "active_dispatch",
        message: `${ticket.ref} still has a live claim or an open dispatch, so hand delivery cannot close it. Release it first, then re-run this closure with the same evidence. Releasing does not discard work already committed. ${forcedClaimReleaseGuidance(ticket.ref, ticket.claim?.by ? String(ticket.claim.by) : void 0)}`,
        ticket
      };
    }
  }
...
function minuteLabel(ms) {
  return Number.isFinite(Number(ms)) ? `${Math.round(Number(ms) / 6e4)}m` : "an unknown time";
}
function claimSkipReason(ticket, now) {
...
function sweepStaleClaims(opts) {
  ...
  ].map((entry) => `${entry.project}\0${entry.ref}`));
  for (const entry of Array.isArray(result.blocked) ? result.blocked : []) {
    if (entry.reason) continue;
    entry.reason = entry.kind === "dirty_shared_tree" ? `its verdict said reclaim, but the shared checkout has paths changed after this dispatch baseline: ${(entry.newlyChangedPaths || entry.paths || []).join(", ") || "none"}. Commit or restore them, then sweep again.` : "its verdict said reclaim, but the shared checkout could not be inspected for uncommitted changes, so the sweep refused to release work it could not see.";
  }
  const skipped = [];
  for (const project of listProjects({ all: true })) {
    ...
    for (const ticket of listTickets(project.slug)) {
      ...
      if (accountedFor.has(`${project.slug}\0${ticket.ref}`)) continue;
...
function externalLinkTicket(slug, idOrRef) {
  const ticket = getTicket(slug, idOrRef);
  if (!ticket) throw new Error(`no ticket "${idOrRef}"`);
  return ticket;
}
function addExternalLink(slug, idOrRef, link) {
  ...
  transaction(() => {
    database().prepare("INSERT OR IGNORE INTO external_links (ticket_id, provider, repo, number, url, created_at) VALUES (?, ?, ?, ?, ?, ?)").run(ticket.id, provider, repo, number, url, (/* @__PURE__ */ new Date()).toISOString());
  });
  return listExternalLinks(slug, { ticketId: ticket.id }).find((row) => row.provider === provider && row.repo === repo && row.number === number);
}
function removeExternalLink(slug, idOrRef, link) {
  const ticket = externalLinkTicket(slug, idOrRef);
  const provider = String(link?.provider || "").trim().toLowerCase();
  const repo = String(link?.repo || "").trim().toLowerCase();
  const number = Number(link?.number);
  if (provider !== "github" || !repo || !Number.isInteger(number) || number < 1) throw new Error("invalid external link");
  database().prepare("DELETE FROM external_links WHERE ticket_id = ? AND provider = ? AND repo = ? AND number = ?").run(ticket.id, provider, repo, number);
  return { ok: true, ticketId: ticket.id };
}
function listExternalLinks(slug, options = {}) {
  ...
  return rows.map((row) => Object.assign({}, row, { number: Number(row.number) }));
}
function parseGitHubIssue(slug, input) {
  ...
  if (match) {
    ...
    repo = remote[1].replace(/\.git$/i, "").toLowerCase();
