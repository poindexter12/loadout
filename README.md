...
| Setup, updates, and missing capabilities recur | [Quartermaster](./plugins/quartermaster) | Guides project setup, keeps active Loadout installs current, checks health, and uses bounded session summaries to suggest one approved improvement at a time. |
...
Use Quartermaster when you want Claude to assess a project, explain which Loadout pieces fit, and leave the rest alone. Install a plugin directly when you already know which job you want.
...
Quartermaster's setup uses project scope by default; Claude Code also supports user and local scopes. The updater follows the recorded scope and project path for every active Loadout install, so a request from one project can update Loadout installs in other recorded projects. Read the update and reload notes before running it.
...
```text
/plugin marketplace add poindexter12/loadout
/plugin install <plugin-name>@loadout --scope project
```

Replace `<plugin-name>` with `sidequest`, `model-gateway`, `observability`, `codebase-mapper`, `live-rules`, or `quartermaster`. Use `--scope user` for a plugin you want in every project, or `--scope local` to keep it out of shared settings. Reload plugins or start a new session after an install. The [plugin guides](https://poindexter12.github.io/loadout/getting-started/) cover the first workflow for each plugin.
...
- [Getting started](https://poindexter12.github.io/loadout/getting-started/)
- [Quartermaster setup guide](https://poindexter12.github.io/loadout/getting-started/quartermaster/)
- [Plugin reference](https://poindexter12.github.io/loadout/reference/)
- [Contributing](https://poindexter12.github.io/loadout/contributing/)
- [Release process](https://poindexter12.github.io/loadout/release-process/)
- [Support](https://poindexter12.github.io/loadout/support/)

## Compatibility

Loadout plugins run inside Claude Code and use its plugin marketplace, plugin scopes, and reload commands. Quartermaster's local scripts use Node.js and the Node standard library. Model Gateway additionally requires the provider access and local process setup described in its guide. No plugin activation opts a project into telemetry, a gateway, or Sidequest routing by itself.

## Attribution

Loadout is a fork of the Eigenwise Toolshed, created by [Eigenwise](https://github.com/Eigenwise), the original author and copyright holder of this codebase. This repository is an independently maintained fork; it is not affiliated with or endorsed by the original author. The original repository is [Eigenwise/eigenwise-toolshed](https://github.com/Eigenwise/eigenwise-toolshed).
