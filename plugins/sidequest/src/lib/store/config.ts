...
// PR delivery channel (US-3). Separate from `delivery`, which stays the git
// method. A stored value always passed this check on write.
function normalizeDeliveryChannel(value?: any) {
  const normalized = normalizeDeliveryChannelRecord(value);
  if ('code' in normalized) throw new Error(normalized.message);
  return normalized;
}

// The one resolver later tickets read: { mode, remote, target } with defaults
// local / origin / the board's integrationBranch. Accepts a board slug, the
// stored project meta, or a boardConfig result. Returns null for an unknown
// slug, like boardConfig.
function resolveDeliveryConfig(board?: any) {
...
function setBoardConfig(slug?: any, patch?: any) {
  return withMetaLock(slug, () => {
    ...
    if (Object.prototype.hasOwnProperty.call(patch, 'deliveryChannel')) {
      // Replaces the whole channel; null clears it back to the local default.
