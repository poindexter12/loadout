...
The plugin keeps the gateway running: its shim supervisor checks the proxy's `/v1/models` endpoint, waits for three consecutive failed probes while the port is bound, and recovers with bounded backoff. `startAll` (the routine behind `ensure`, `setup`, and SessionStart) applies that same three-probe patience before deciding the shim itself is down, so a single bad `/healthz` check can no longer send a healthy supervisor a SIGTERM. A healthy proxy is left alone. Startup, stop, supervisor replacement, drain, and worker restart require confirmed shim listener ownership before changing processes or sending control requests. A bound port with an unavailable PID or inconclusive process inspection is left untouched, not treated as free. Retry `ensure` or `stop` after process inspection becomes available; do not kill an unidentified listener to clear the port. Recovery retries unknown ownership automatically after backoff. A newer Model Gateway cache version can replace a confirmed older sibling from the same marketplace and plugin name. An older session leaves a newer shim running and tells you to reload plugins or restart Claude Code. A different marketplace, plugin name, or non-cache install stays foreign; manage it through its own install. Account-specific state and sockets remain separate. Sidequest can select gateway models automatically when both plugins are installed.

Two `ensure` invocations can start close together — two SessionStart hooks firing back to back, for example. `ensure` claims an exclusive lockfile (`$CLAUDE_CONFIG_DIR/model-gateway/ensure.lock`) before it decides whether recovery is needed; a second, concurrent `ensure` waits briefly for the first to finish and reports that invocation's actual outcome instead of independently observing the same symptom and starting its own recovery. A lock left behind by a process that is no longer alive is reclaimed rather than blocked on forever, and so is a lock older than an absolute age cap, whatever its recorded pid reports — a dead holder whose pid has been recycled onto an unrelated live process therefore cannot deadlock recovery. Releasing frees only a lock the releasing process still holds, so a holder that was reclaimed from cannot delete its successor's lock. This closes a 2026-09-14 incident where two concurrent `ensure` processes each decided, on a single bad probe, that a supervisor which had been healthy for 16 hours needed to be torn down.
...
The shim writes request-route metadata to `$CLAUDE_CONFIG_DIR/model-gateway/logs/request-routes.jsonl` (the active config dir, default `~/.claude`) by default. Each JSONL record contains the time, backend, model, request path, and, when available, route, effort, session, agent, parent-agent, dispatch-marker length, and session-source fields. It does not write request bodies, prompts, messages, tools, authentication, or arbitrary headers. Set `CODEX_GATEWAY_REQUEST_LOG=0` before the shim starts, then restart the shim through the normal setup or ensure path, to disable this route log. The setting is read by the shim process at startup, not by `/reload-plugins` in an already-running process. The path can be changed with `CODEX_GATEWAY_REQUEST_LOG_PATH`.

For each session, usage observability also keeps a small high-water file under `$CLAUDE_CONFIG_DIR/model-gateway/request-body/`. The filename is derived from the session id. The JSON contains only the largest forwarded request-body byte count seen for that session and its observation time. It does not contain the request body. The gateway does not promise a retention period for either record; manage or delete local files according to your own policy.

When the gateway disappears or restarts, ask Claude to run `doctor`. It names the lifecycle evidence at `$CLAUDE_CONFIG_DIR/model-gateway/logs/lifecycle.jsonl` and distinguishes an observed supervisor, worker, or proxy exit from no exit evidence. The bounded records identify PIDs, orderly setup/stop/restart requests, signals, and recovery outcomes. A force-killed supervisor or OS termination can leave no final record, so a missing exit entry does not prove an orderly shutdown.
