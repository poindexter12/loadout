...
- **Codex requests fail with `400 Invalid schema for function '<Tool>'`**: OpenAI validates every
  tool's JSON Schema `pattern` against ECMA-262 and refuses Unicode property escapes (`\p{...}`)
  and lookbehind. A single offending tool fails the whole request before it reaches the model, and
  Claude Code's Artifact tool carries such a pattern on its `field` parameter. The shim strips only
  the unsupported `pattern` values from Codex-bound tool schemas, leaving parseable ones intact so
  the model still sees the useful constraints; a malformed argument then fails at the tool instead
  of at the API. Claude-bound requests are never touched. If it recurs, read the tool name out of
  the error and confirm the shim actually restarted (`stop` + `start`).
