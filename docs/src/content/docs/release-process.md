...
A local-only tag that already names a planned version stops the cut. The cut classifies it by the annotated tag's tagger against this clone's release identity (the committer identity plus any `loadout.releaseTagger` config values). A foreign tag, fetched from another remote, gets a `git tag -d` plus a `tagOpt --no-tags` fix for that remote.
