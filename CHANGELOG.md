...
- Say why the claim sweep left a claim alone, and let the control plane reclaim one under its own name (SQ-69)
...
- Resolve the verification capture board from the dispatched ticket, not the inherited worktree (SQ-70)
...
- Cancel the upstream request when a client aborts (SQ-29) [`3a6385e`](https://github.com/poindexter12/loadout/commit/3a6385ed)
...
- Name why the /v1/models probe failed instead of reporting a bare false (SQ-63)
...
- Derive the full-suite phase budget from capacity the machine actually has, not the cores it owns (SQ-62)
